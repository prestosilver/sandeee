...
pub fn convert(b: *std.Build, in: []const www.DiskFile) !std.ArrayList(u8) {
    ...
    for (in) |item| {
        ...
        const data: std.ArrayList(u8) = if (item.converter) |converter|
            try converter(b, item.input)
        else blk: {
            const tmp: std.ArrayList(u8) = item.getData(b);
