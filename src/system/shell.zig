...
pub const Shell = struct {
    ...
    //var allocP = std.fmt.allocPrint(allocator.alloc, "\\0x{X}", .{ch}) catch "";
    //defer allocator.alloc.free(allocP);
    //result.data.appendSlice(allocP) catch {};
    ...
    pub fn run(self: *Shell, cmd: []const u8, params: []const u8) Result {
        ...
        if (check(cmd, "cmd")) return self.todo(params);
