...
fn runFileInFolder(self: *Shell, root: files.FolderLink, cmd: []const u8, param: *Params) !Result {
    ...
    const file = folder.getFile(cmd) catch |err| {
        ...
        const cmdeep = try std.fmt.allocPrint(allocator, "{s}.eep", .{cmd});
        defer allocator.free(cmdeep);
    ...
    const new_cmd = try std.mem.concat(allocator, u8, &.{
        open_cmd,
        " ",
        cmd,
        " ",
...
pub fn getVMResult(self: *Shell) !?Result {
    if (self.vm) |vm_handle| {
        const data = try Vm.Manager.instance.getOutput(vm_handle);
        defer data.deinit();
...
pub fn appendVMIn(self: *Shell, char: u8) !void {
    if (self.vm) |vm_handle| {
        try Vm.Manager.instance.appendInputSlice(vm_handle, &.{char});
    }
}
...
fn runAsm(self: *Shell, root: files.FolderLink, cmd: []const u8, params: *Params) !Result {
    ...
    while (file) |item| : (file = item.next_sibling) {
        if (std.mem.eql(u8, item.name[rootlen..], cmd)) {
            ...
            self.vm = try Vm.Manager.instance.spawn(self.root, params.buffer, ops);
                ...
                while (param.next()) |in_param| {
                    if (std.mem.eql(u8, in_param, ":file")) {
                        ...
                        if (url_data == null) {
                            ...
                            return .{
                                .data = try allocator.dupe(u8, "Invalid web call with 2 urls"),
                            };
                        }
                    }
                }
    ...
    .{ "logout", ShellCommand{
        ...
        .func = struct {
            fn logout(_: *Shell, _: *Params) !Result {
                const adds = try allocator.create(popups.quit.PopupQuit);
                adds.* = .{
                    .shader = shader,
                    .icons = .{
                        .atlas("bar", .{
                ...
                return .{
                    .data = try allocator.dupe(u8, "Launched"),
                };
            }
        }.logout,
    } },
...
pub fn deinit(self: *Shell) void {
    if (self.vm) |vm_handle| {
        Vm.Manager.instance.destroy(vm_handle);
        self.vm = null;
    }
}
