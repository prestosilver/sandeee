...
stack: [STACK_MAX]VmAllocator.ObjectRef = undefined,
...
pub inline fn pushStack(self: *Vm, entry: VmAllocator.ObjectRef) VmError!void {
...
pub inline fn pushStackI(self: *Vm, value: u64) VmError!void {
    ...
    self.stack[self.rsp] = try VmAllocator.new(.{ .value = value });
    self.rsp += 1;
}
...
pub inline fn popStack(self: *Vm) VmError!VmAllocator.ObjectRef {
    if (self.rsp == 0) return error.StackUnderflow;
    self.rsp -= 1;
    return self.stack[self.rsp];
}

pub inline fn findStack(self: *Vm, idx: u64) VmError!VmAllocator.ObjectRef {
    if (self.rsp <= idx) return error.StackUnderflow;
    return self.stack[self.rsp - 1 - @as(usize, @intCast(idx))];
}

pub inline fn replaceStack(self: *Vm, a: VmAllocator.ObjectRef, b: VmAllocator.ObjectRef) VmError!void {
...
pub inline fn runOp(self: *Vm, op: Operation) VmError!void {
    ...
    switch (op.code) {
        ...
        .Size => {
            ...
            const old = try std.fmt.allocPrint(allocator.alloc, "{}", .{old_rope});
        ...
        .Disc => {
            ...
            switch (op.value.?) {
                0 => {
                    ...
                    std.mem.copyForwards(VmAllocator.ObjectRef, self.stack[self.rsp .. self.rsp + items.len], items);
                    self.rsp += items.len;
                },
            }
        },
        ...
        .Mem => {
            ...
            if (VmAllocator.find(a.data().value)) |obj| {
