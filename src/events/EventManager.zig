...
subs: std.StringHashMap([]Listener(*void)) = std.StringHashMap([]Listener(*void)).init(allocator),

pub fn deinit() void {
    var iter = instance.subs.iterator();
    while (iter.next()) |item| {
        allocator.free(@as([]Listener(*void), item.value_ptr.*));
    }
...
pub fn registerListener(self: *Manager, comptime T: type, callee: *const fn (T) anyerror!void) !void {
    ...
    if (self.subs.getPtr(@typeName(T))) |list| {
        ...
        const list = try allocator.alloc(Listener(*void), 1);
